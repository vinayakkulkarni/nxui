...
const rotateY = computed(() => {
  ...
  return ((elementX.value - rect.width / 2) / (rect.width / 2)) * props.maxTilt;
});

const rotateX = computed(() => {
  if (isOutside.value || !containerRef.value) return 0;
  const rect = containerRef.value.getBoundingClientRect();
  return -((elementY.value - rect.height / 2) / (rect.height / 2)) * props.maxTilt;
});
...
<template>
  ...
  >
    <div class="absolute inset-0 pointer-events-none transition-opacity duration-300" :style="glareStyle" />
    <div class="absolute pointer-events-none transition-opacity duration-300" :style="spotlightStyle" />
