  ...
  const props = withDefaults(
    ...
    }>(),
    {
      items: () => [
        ...
        { image: 'https://i.pravatar.cc/300?img=11', title: 'Jordan Chen', subtitle: 'DevOps Engineer', handle: '@jordanchen', borderColor: '#10B981', gradient: 'linear-gradient(210deg, #10B981, #000)', url: 'https://linkedin.com/' },
  ...
  >
    <article
      ...
      :style="{
        ...
        '--card-gradient': item.gradient || 'linear-gradient(145deg, #333, #000)',
    ...
    >
      <div class="chroma-img-wrapper">
        <img :src="item.image" :alt="item.title" loading="lazy" class="size-full rounded-lg object-cover" />
      </div>
      <footer class="chroma-info">
        <h3 class="text-sm font-semibold text-white">{{ item.title }}</h3>
        <span v-if="item.handle" class="text-xs text-[#aaa]">{{ item.handle }}</span>
    ...
    <!-- Fade overlay -->
    <div class="chroma-fade" :style="{ opacity: fadeOpacity }" />
  </div>
</template>
...
.chroma-overlay {
  ...
  mask-image: radial-gradient(circle var(--r) at var(--x) var(--y), transparent 0%, transparent 15%, rgba(0, 0, 0, 0.1) 30%, rgba(0, 0, 0, 0.35) 60%, rgba(0, 0, 0, 0.68) 88%, white 100%);
  -webkit-mask-image: radial-gradient(circle var(--r) at var(--x) var(--y), transparent 0%, transparent 15%, rgba(0, 0, 0, 0.1) 30%, rgba(0, 0, 0, 0.35) 60%, rgba(0, 0, 0, 0.68) 88%, white 100%);
}
...
.chroma-fade {
  ...
  -webkit-backdrop-filter: grayscale(1) brightness(0.78);
  background: rgba(0, 0, 0, 0.001);
  mask-image: radial-gradient(circle var(--r) at var(--x) var(--y), white 0%, white 15%, rgba(255, 255, 255, 0.78) 45%, rgba(255, 255, 255, 0.5) 75%, transparent 100%);
  -webkit-mask-image: radial-gradient(circle var(--r) at var(--x) var(--y), white 0%, white 15%, rgba(255, 255, 255, 0.78) 45%, rgba(255, 255, 255, 0.5) 75%, transparent 100%);
