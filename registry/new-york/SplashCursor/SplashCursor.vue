<script setup lang="ts">
  ...
  function init() {
    ...
    interface Pointer {
      ...
      deltaX: number; deltaY: number; down: boolean; moved: boolean;
    ...
    const halfFloatTexType = isWebGL2 ? gl.HALF_FLOAT : (gl.getExtension('OES_texture_half_float')?.HALF_FLOAT_OES ?? gl.HALF_FLOAT);
    ...
    // Programs
    class Prog { uniforms: Record<string, WebGLUniformLocation | null>; program: WebGLProgram;
      constructor(vs: WebGLShader, fs: WebGLShader) { this.program = createProgram(vs, fs); this.uniforms = getUniforms(this.program); }
      bind() { gl!.useProgram(this.program); }
    }

    class Mat { vertexShader: WebGLShader; fragmentShaderSource: string; programs: Record<number, WebGLProgram> = {}; activeProgram: WebGLProgram | null = null; uniforms: Record<string, WebGLUniformLocation | null> = {};
      constructor(vs: WebGLShader, fss: string) { this.vertexShader = vs; this.fragmentShaderSource = fss; }
      setKeywords(keywords: string[]) {
        let hash = 0; for (const k of keywords) hash += hashCode(k);
        if (!this.programs[hash]) { const fs = compileShader(gl!.FRAGMENT_SHADER, this.fragmentShaderSource, keywords); this.programs[hash] = createProgram(this.vertexShader, fs); }
        if (this.programs[hash] === this.activeProgram) return;
        this.uniforms = getUniforms(this.programs[hash]); this.activeProgram = this.programs[hash];
      }
    ...
    const gradientSubtractProgram = new Prog(baseVertexShader, gradientSubtractShader);
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (!p.color || (p.color.r === 0 && p.color.g === 0 && p.color.b === 0)) p.color = generateColor();
    };
    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      const color = { ...p.color }; color.r *= 10; color.g *= 10; color.b *= 10;
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      for (let i = 0; i < touches.length; i++) {
        const { x: posX, y: posY } = getLocalCoords(touches[i].clientX, touches[i].clientY);
        p.prevTexcoordX = p.texcoordX;
        p.prevTexcoordY = p.texcoordY;
        p.texcoordX = posX / canvas!.width;
        p.texcoordY = 1.0 - posY / canvas!.height;
