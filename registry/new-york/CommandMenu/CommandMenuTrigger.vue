...
<template>
  <button
    ...
    @click="emit('open')"
  >
    <Icon name="lucide:search" class="h-4 w-4 opacity-70 group-hover:opacity-100 transition-opacity" />
    <span class="hidden lg:inline-flex">{{ triggerLabel }}</span>
    <span class="inline-flex lg:hidden">Search</span>
    <kbd class="pointer-events-none absolute right-1.5 top-1.5 hidden h-6 select-none items-center gap-0.5 rounded-md border bg-background/80 px-1.5 font-mono text-[10px] font-medium text-muted-foreground/70 sm:flex">
