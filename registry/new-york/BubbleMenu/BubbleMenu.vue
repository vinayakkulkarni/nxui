<script setup lang="ts">
  ...
  const props = withDefaults(
    ...
    {
      items: () => [
        { label: 'home', href: '#', rotation: -8, hoverBgColor: '#3b82f6', hoverTextColor: '#fff' },
        { label: 'about', href: '#', rotation: 8, hoverBgColor: '#10b981', hoverTextColor: '#fff' },
        { label: 'projects', href: '#', rotation: 8, hoverBgColor: '#f59e0b', hoverTextColor: '#fff' },
        { label: 'blog', href: '#', rotation: 8, hoverBgColor: '#ef4444', hoverTextColor: '#fff' },
        { label: 'contact', href: '#', rotation: -8, hoverBgColor: '#8b5cf6', hoverTextColor: '#fff' },
      ],
      menuBg: '#ffffff',
...
<template>
  <div :class="cn('relative size-full', $props.class)">
    ...
    <nav class="absolute inset-x-0 top-4 z-[99] flex items-center justify-between px-8 pointer-events-none">
      <div
        class="pointer-events-auto inline-flex h-14 items-center rounded-full px-4 shadow-lg"
        :style="{ background: menuBg }"
      >
        <span class="inline-flex w-[120px] items-center justify-center text-sm font-semibold" :style="{ color: menuContentColor }">
    ...
    >
      <ul class="m-0 flex w-full max-w-[1600px] list-none flex-wrap justify-stretch gap-y-1 px-6">
        <li
        ...
        >
          ...
          >
            <span class="bubble-pill-label" :class="isOpen ? 'bubble-pill-label--visible' : ''">
              {{ item.label }}
            </span>
          </a>
        </li>
...
.bubble-pill {
  ...
  animation: bubble-in var(--duration) cubic-bezier(0.34, 1.56, 0.64, 1) var(--stagger) forwards;
