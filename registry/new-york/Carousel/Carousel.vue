<script setup lang="ts">
  ...
  const props = withDefaults(
    ...
    }>(),
    {
      items: () => [
        ...
        {
          title: 'Backgrounds',
          description: 'Beautiful backgrounds for your projects.',
          id: 4,
        },
        {
          title: 'Common UI',
  ...
  const activeIndex = computed(() => {
    ...
    if (props.loop)
      return (position.value - 1 + props.items.length) % props.items.length;
  ...
  function startAutoplay() {
    ...
    autoplayTimer = setInterval(() => {
      ...
      updatePosition(
        Math.min(position.value + 1, itemsForRender.value.length - 1),
      );
    }, props.autoplayDelay);
  }
  ...
  >
    ...
    >
      <div
        ...
        :class="
          round ? 'items-center justify-center text-center rounded-full' : ''
        "
        :style="{
          width: `${itemWidth}px`,
      ...
      >
        <div class="mb-4 p-5 pt-5">
          <span
            class="flex size-7 items-center justify-center rounded-full bg-foreground"
          >
            <slot :name="`icon-${idx}`">
              <svg
        ...
        <div class="p-5 pb-5">
          <div class="mb-1 text-lg font-black text-foreground">
            {{ item.title }}
          </div>
    ...
    <div
      ...
      :class="round ? 'absolute bottom-12 left-1/2 z-10 -translate-x-1/2' : ''"
    >
