...
vec4 cppn_fn(vec2 coordinate,float in0,float in1,float in2){
  buf[6]=vec4(coordinate.x,coordinate.y,0.395+in0,0.36+in1);
  buf[7]=vec4(0.14+in2,sqrt(coordinate.x*coordinate.x+coordinate.y*coordinate.y),0.,0.);
  buf[0]=mat4(vec4(6.54,-3.61,0.76,-1.14),vec4(2.46,3.17,1.22,0.06),vec4(-5.48,-6.16,1.87,-4.77),vec4(6.04,-5.54,-0.91,3.25))*buf[6]+mat4(vec4(0.85,-5.72,3.98,1.65),vec4(-0.24,0.58,-1.77,-5.35),vec4(0,0,0,0),vec4(0,0,0,0))*buf[7]+vec4(0.22,1.12,-1.80,5.03);
  buf[1]=mat4(vec4(-3.35,-6.06,0.56,-4.47),vec4(0.86,1.74,5.64,1.61),vec4(2.49,-3.50,1.72,6.36),vec4(3.31,8.21,1.14,-1.17))*buf[6]+mat4(vec4(5.24,-13.03,0.01,15.87),vec4(2.99,3.13,-0.89,-1.68),vec4(0,0,0,0),vec4(0,0,0,0))*buf[7]+vec4(-5.95,-6.57,-0.88,1.54);
  buf[0]=sigmoid(buf[0]);buf[1]=sigmoid(buf[1]);
  buf[2]=mat4(vec4(-15.22,8.10,-2.43,-1.94),vec4(-5.95,4.31,2.64,1.27),vec4(-7.31,6.73,5.25,5.94),vec4(5.08,8.98,-1.73,-1.16))*buf[6]+mat4(vec4(-11.97,-11.61,6.15,11.24),vec4(2.12,-6.26,-1.71,-0.70),vec4(0,0,0,0),vec4(0,0,0,0))*buf[7]+vec4(-4.17,-3.23,-4.58,-3.64);
  buf[3]=mat4(vec4(3.18,-13.74,1.88,3.23),vec4(0.64,12.77,1.91,0.51),vec4(-0.05,4.48,1.47,1.80),vec4(5.00,13.00,3.40,-4.56))*buf[6]+mat4(vec4(-0.13,7.72,-3.14,4.74),vec4(0.64,3.71,-0.81,-0.39),vec4(0,0,0,0),vec4(0,0,0,0))*buf[7]+vec4(-1.18,-21.62,0.79,1.23);
...
void main(){
  ...
  float sl=sin(gl_FragCoord.y*uScanFreq)*0.5+0.5;
  col.rgb*=1.-(sl*sl)*uScan;
