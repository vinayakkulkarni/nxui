...
<template>
  <div :class="cn('w-full max-w-sm rounded-xl border bg-card p-5 shadow-sm', props.class)">
    <div class="mb-4 flex items-center justify-between">
      <div class="text-sm font-medium text-muted-foreground">
        {{ props.flight.airline }} · {{ props.flight.flightNumber }}
      </div>
      <span :class="cn('rounded-full px-2.5 py-0.5 text-xs font-medium', statusColors[props.flight.status])">
    ...
    <div class="mb-4 flex items-center justify-between">
      <div class="text-left">
        ...
        <div class="text-xs text-muted-foreground">{{ props.flight.departure.city }}</div>
      ...
      <div class="relative mx-4 flex flex-1 items-center">
        ...
        >
          <Icon name="lucide:plane" class="size-4 -translate-x-1/2 -translate-y-1/2 text-primary" />
        </component>
      </div>
      <div class="text-right">
        <div class="text-2xl font-bold">{{ props.flight.arrival.code }}</div>
        <div class="text-xs text-muted-foreground">{{ props.flight.arrival.city }}</div>
    ...
    <div v-if="props.flight.gate" class="text-right text-xs text-muted-foreground">
