---
title: Dithering
description: Animated 2-color dithering over multiple pattern sources (noise, warp, dots, waves, ripple, swirl, sphere). Ported from Paper Shaders (Apache-2.0).
---

::demo-paper-dithering
::

## Installation

```bash
npx shadcn-vue@latest add "https://nxui.geoql.in/r/paper-dithering.json"
```
...
| `color-front`  | `string`                                                                     | `'#00b2ff'` | Foreground (ink) color in any CSS color format     |
| `shape`        | `'simplex' \| 'warp' \| 'dots' \| 'wave' \| 'ripple' \| 'swirl' \| 'sphere'` | `'sphere'`  | Shape pattern used to drive the dither threshold   |
| `type`         | `'random' \| '2x2' \| '4x4' \| '8x8'`                                        | `'4x4'`     | Dithering algorithm applied to the shape           |
| `size`         | `number`                                                                     | `2`         | Pixel size of the dithering grid (0.5 to 20)       |
